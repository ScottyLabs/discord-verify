...
/// Register the setunverifiedrole command
pub fn register() -> CreateCommand<'static> {
    CreateCommand::new("setunverifiedrole")
        .description("Set the unverified role for this server")
        .add_option(
            CreateCommandOption::new(
                CommandOptionType::Role,
                "role",
                "The role to assign when users join before they verify",
            )
            .required(true),
        )
}

/// Handle the setunverifiedrole command
pub async fn handle(
    ctx: &Context,
    ...
    if !is_admin(ctx, &command.member, guild_id, user.id).await? {
        let response = CreateInteractionResponse::Message(
            CreateInteractionResponseMessage::new()
                .content("You need administrator permissions to configure the unverified role.")
                .ephemeral(true),
        );
        command.create_response(&ctx.http, response).await?;
        return Ok(());
    ...
    if role.id == everyone_role_id {
        let response = CreateInteractionResponse::Message(
            CreateInteractionResponseMessage::new()
                .content("You cannot set @everyone as the unverified role.")
                .ephemeral(true),
        );
        command.create_response(&ctx.http, response).await?;
        return Ok(());
    ...
    if role.managed() {
        let response = CreateInteractionResponse::Message(
            CreateInteractionResponseMessage::new()
                .content(
                    "You cannot use a managed role (bot/integration role) as the unverified role.",
                )
                .ephemeral(true),
        );
        command.create_response(&ctx.http, response).await?;
    ...
    let response = CreateInteractionResponse::Message(
        CreateInteractionResponseMessage::new()
            .content(format!(
                "Unverified role has been set to {}. Users who join will receive this role until they verify.",
                role.mention()
            ))
            .ephemeral(true),
    );
